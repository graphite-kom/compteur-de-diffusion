<?php
	...
	$app->get('/record_diffusion/:data_string', function($data_string) use ($app){
		...
		try{
			...
			$data 		= get_json_decoded($data_string);
			
			if(is_object($data)){
			
				$nim 		= checkParam($data->shop, 'nim');
				
				$caisse_num	= checkParam($data->machine, 'caisse_num');
				
				$post_obj 	= checkParam(str_replace("|", "/", $data->post_obj), 'post_obj');
				
				$cn_fix_id 	= checkParam($data->cn_fix_id, 'cn_fix_id');
			
			}else{
				
				throw new Exception("Invalid JSON format : ".var_export($data, true));
